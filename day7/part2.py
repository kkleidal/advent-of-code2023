...
def hand_classifier(hand):
    ...
    counts = dict(Counter(hand_without_jokers))

    # Heuristic: adding jokers to the max class is always best:
    if len(counts) == 0:
        # all jokers
        counts[14] = jokers
    else:
        to_card, _ = max(counts.items(), key=lambda kvp: (kvp[1], kvp[0]))
        counts[to_card] += jokers
