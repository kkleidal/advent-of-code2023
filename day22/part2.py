...
def do_drop():
    ...
    for i in list(nx.topological_sort(supports)):
        if blocks[i].drop(possibly_below):
            drops += 1
...
total = 0
for i in reversed(list(nx.topological_sort(actually_supports))):
    dropped = {i}
    q = deque([i])
    while q:
        j = q.popleft()
        supported_by = {k for k, _ in actually_supports.in_edges(j)}
        if supported_by.issubset(dropped) or j == i:
            dropped.add(j)
            for _, k in actually_supports.out_edges(j):
                q.append(k)
    dropped = dropped - {i}
    total += len(dropped)
print(total)
